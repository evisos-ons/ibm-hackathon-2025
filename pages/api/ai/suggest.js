...

{
  ...
  "environmental": "Environmental impact analysis, written in plain text"${
    price
      ? ',\n  "price": "Analysis of the price paid (£' +
        price +
        ') compared to typical market prices for similar products, including whether it appears to be good value"'
      : ""
  }
}
...
Remember to format the response as a valid JSON object with the exact keys specified above.`;
    ...
    try {
      ...
      suggestions = {
        recycling:
          text.match(/recycling["']?\s*:\s*["']([^"']*)["']/i)?.[1] ||
          text
            .match(/Recycling Instructions:?(.*?)(?=Health Analysis:|$)/s)?.[1]
            ?.trim() ||
          "",

        health:
          text.match(/health["']?\s*:\s*["']([^"']*)["']/i)?.[1] ||
          text
            .match(/Health Analysis:?(.*?)(?=Alternative Suggestions:|$)/s)?.[1]
            ?.trim() ||
          "",

        alternatives:
          text.match(/alternatives["']?\s*:\s*["']([^"']*)["']/i)?.[1] ||
          text
            .match(/Alternative Suggestions:?(.*?)(?=Usage Tips:|$)/s)?.[1]
            ?.trim() ||
          "",

        usage:
          text.match(/usage["']?\s*:\s*["']([^"']*)["']/i)?.[1] ||
          text
            .match(/Usage Tips:?(.*?)(?=Environmental Impact:|$)/s)?.[1]
            ?.trim() ||
          "",

        environmental:
          text.match(/environmental["']?\s*:\s*["']([^"']*)["']/i)?.[1] ||
          text
            .match(/Environmental Impact:?(.*?)(?=Price Analysis:|$)/s)?.[1]
            ?.trim() ||
          "",

        price: price
          ? text.match(/price["']?\s*:\s*["']([^"']*)["']/i)?.[1] ||
            text.match(/Price Analysis:?(.*?)$/s)?.[1]?.trim() ||
            ""
          : undefined,
      };
